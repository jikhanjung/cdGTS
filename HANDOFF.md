...
**Last updated**: 2026-07-18. 운영 `cdgts.paleobytes.info`(dolfinid-2) @ **0.1.70** · 테스트 서버 **m710q**(tailscale serve → `127.0.0.1:8011`) @ **0.1.70**. 이미지 `honestjung/cdgts:0.1.35~0.1.70` dockerhub push 완료. 공개 `https://cdgts.paleobytes.info/healthz` = 0.1.70/node_types 16(boundaries 177·graphs 7). **양 서버 cdgts + cdgts-worker 둘 다 가동, 비-root(prod uid 1001·test uid 1000), DB = `/srv/cdGTS/db/` 서브디렉터리 마운트**.
...
- **배포/운영** (배포·데이터 계약 P08 — [DEPLOY.md](DEPLOY.md)·[deploy/README.md](deploy/README.md)·[deploy/deploy.toml](deploy/deploy.toml)):
  ...
  - **운영서버** `cdgts.paleobytes.info`(GCP dolfinid-2) @ **0.1.70**(nginx + certbot). 테스트 `127.0.0.1:8011`(m710q) @ **0.1.70**.
