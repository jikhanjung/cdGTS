...
**Last updated**: 2026-07-04 (**v0.1.4 운영서버 배포 완료**. 개념 코퍼스 14주제 + 스키마 v0 위에
앱 아키텍처 → 개발계획 P01 → **Phase 0~6 구현 → 계산 커널 → Docker 배포 → 배포/DB 분리·원자적 sync·NAS 백업
→ 예제 그래프 3종 → 릴리스 diff UI → 노드 속성 인스펙터/결과 패널 → seed 통합 → ICC 테이블 bake → ICS
chart.ttl 시드**. 스택 확정: **Django 5.2.12 + SQLite + DRF 3.17 + React Flow(Vite)**.
5개 앱[chrono·nodes·graph·engine·releases] + 프론트 에디터/ICC 테이블/diff 뷰. 백엔드 **pytest 65 passed**.
운영 **cdgts.paleobytes.info @ 0.1.4**(통합 seed 2026.07.0 + ICS chart.ttl 확장 재시드 완료), 개발/테스트 **@ 0.1.1**. devlog 001~038 push.)
...
- **앱 5개** (의존: chrono ◁ nodes ◁ graph ◁ engine ◁ releases):
  - `chrono` — 정본 registry(Unit 이중명명·Boundary·Lineage·Authority·Ratification·Locality).
    fixture **ICS chart.ttl 확장**: Unit 42 · Boundary 175(Period 이상은 네트웍, Epoch/Age 이하는 registry).
  - `nodes` — 타입 시스템(NodeType **13** + Port) + `Distribution` 값객체(충실도 L0–L5). `published-age`(공표값 leaf) 포함.
  - `graph` — DAG(Graph·NodeInstance[+description·width]·Edge·NodeGroup·Gateway) + DAG 불변식 +
    DRF `GET/PUT /api/graphs/{id}` React Flow 왕복. fixture **예제 그래프 3종** + `example-icc-partial`
    (합본 + Period 이상 공표값 데이터 노드 → 36경계 게이트웨이).
  ...
  - `releases` — ModelCandidate·Release(selection+clamps)·BoundaryRecord(bake)·Clamp + bake/diff API
    (`/api/releases/{id}/bake/`, `/diff/?a=&b=`) + **`bake_graph` (그래프→ICC 테이블) `POST /api/graphs/{id}/bake/`**.
    fixture 예제 릴리스 2종(값 diff + 토폴로지 diff 데모).
- **프론트**: 팔레트 → 캔버스 drag&drop → 저장(PUT)/평가 + 결과 뱃지, **그래프 선택 드롭다운**,
  **노드 속성 인스펙터**(스키마 기반)·**결과 패널**·노드 리사이즈/description 툴팁,
  **ICC 테이블 뷰**(그래프 bake → 경계 스냅샷), **릴리스 Diff 뷰**(값/토폴로지 직교, 인라인 bake). `npm run build` 통과.
- **배포/운영**:
  - Docker 이미지 `honestjung/cdgts`(numpy/scipy 포함), `deploy/build.sh <ver>` 로 pytest→bump→build→push.
  - **운영서버** `cdgts.paleobytes.info` @ **0.1.4**(nginx + certbot). **개발/테스트** `127.0.0.1:8011` @ 0.1.1.
...
- **테스트**: 백엔드 `pytest` **65 passed**. 테스트 fixture 는 seed 파일(`01_chrono`·`02_nodes`)을 loaddata.
  seed 회귀 테스트 `releases/test_seed.py`(데이터 있는 DB replace·self-FK·add 멱등·정본값 가드·bake_graph 36경계·finer=registry-only).
- **문서 코퍼스**: `docs/` 14주제 × 한/영 + README + app-architecture 한/영. 진입점 `docs/concept-map.md`.
  최신 `tier-category-model`(레이어 0–6 → 티어 × 카테고리 회고).
...
- **노드 UX/예제**(034·035) — 노드 폭 축소·리사이즈·description 툴팁(migration graph.0002) + 조립 예제 `example-icc-partial`.
- **ICC 테이블**(036·037) — `bake_graph`(그래프 게이트웨이→경계 스냅샷) + ICC 테이블 뷰. **ICS chart.ttl 시드**:
  Period 이상 공표값 데이터 노드(예제 3개는 파이프라인)·Epoch/Age registry(units 42·boundaries 175·`published-age` 타입).
- **개념 회고** — `tier-category-model`(레이어 0–6 → 티어 × 카테고리).
- **릴리스**(038) — **v0.1.4 운영 배포 + `seed --mode=replace` 재시드**(ICC 확장 반영, add 는 그래프 원자 skip 이라 replace 필수).
...
- [x] **v0.1.4 배포 + ICC 재시드**(devlog 038) — ICC 테이블 bake + ICS chart.ttl 시드. 운영 `seed --mode=replace`.
- [ ] **`engine._certify` 층서순 정합** — 현재 게이트웨이 나열순 monotonicity 스텁 → 36경계 그래프에서 warn.
      층서(연대)순 정렬 후 판정 필요.
- [ ] **finer 경계 값 릴리스화** — Epoch/Age 공표값을 candidate/selection → 완전한 공표 ICC 릴리스(현재 registry note 만).
- [ ] **브라우저 육안 검증** — 프론트 drag&drop·엣지·복원·결과뱃지·인스펙터·ICC 테이블·diff 뷰 실제 클릭 확인(헤드리스 미검증분).
