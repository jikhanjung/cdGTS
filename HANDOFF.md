...
백엔드 **pytest 114 passed**. devlog 001~111 + 리뷰/계획(R01·P04·P05) push. P04(불변 Bake·Vault) + **P05(멀티유저 CI .1~.5) 전체** → **테스트 서버 0.1.29 배포·검증**(로그인·fork·propose·ratify·sandbox override 실동작 + 가시성 취약점 차단). 운영은 0.1.25.
...
- **배포/운영**:
  ...
  - **운영서버** `cdgts.paleobytes.info` @ **0.1.25**(nginx + certbot). 개발/테스트 `127.0.0.1:8011` @ **0.1.29**(P04+P05).
...
- **[P05](devlog/20260707_P05_arc-c-multiuser-ci-platform.md)** — 아크 C: 멀티유저 CI. 결정 5개 확정, 인터벌
  ...
  - **P05 전체 완료 · 테스트 0.1.29 배포·검증**. 흐름: 로그인 → fork → 편집 → (bake→Vault) / propose → review →
