...
**Last updated**: 2026-07-13. 운영 `cdgts.paleobytes.info`(dolfinid-2) @ **0.1.57** · 테스트 서버 **m710q**(tailscale serve → `127.0.0.1:8011`) @ **0.1.58**. 이미지 `honestjung/cdgts:0.1.35~0.1.58` dockerhub push 완료.
- **0.1.58 부트스트랩 self-heal — prod 에서 repo 영영 불필요**([커밋 14f1fd8]): `_extract_and_deploy.sh` 가 매 배포마다 부트스트랩 파일도 이미지에서 갱신(래퍼=exec 후 즉시 덮어쓰기 · 자기 자신=임시파일→원자 rename, 다음 배포부터). **git-free 부트스트랩**(repo 없이 `docker create`+`docker cp /app/deploy/host/{_extract_and_deploy,deploy-prod,deploy-dev}.sh`) 절차 DEPLOY.md 명시. m710q 에서 실검증: repo 미사용 부트스트랩 → `deploy-dev.sh 0.1.58` → `self-heal *` 3줄 출력 + smoke green(0.1.58). **prod 는 0.1.57 → 0.1.58 로 올릴 때 최초 1회 git-free 부트스트랩 후 self-heal 활성**(이후 repo 삭제 가능).
