...
- **0.1.56 이미지 빌드·푸시·검증 완료(미배포)** — P08.1~P08.6(배포·데이터 계약 retrofit) 전량 포함. 실 0.1.56 컨테이너로 end-to-end 검증: 빈 DB→`/healthz` 503, seed(replace, inserted 1850)→200/버전 0.1.56, `smoke.sh` 양성 pass·버전 불일치 음성 fail, git-free 추출(host 파일 5개) 확인. **배포는 사용자** — ⚠️ **0.1.56 1회 부트스트랩**: 호스트 래퍼가 옛 2줄 버전이라 이번만 repo 머신에서 `./deploy/sync_to_srv.sh` 1회 후 `deploy-{dev,prod}.sh 0.1.56`. 시드 변경(P08.1) 포함 → 배포 후 `seed --mode=replace` + `seed_demo`.
