...
**Last updated**: 2026-07-13. 운영 `cdgts.paleobytes.info` @ **0.1.55** 배포 완료 · 테스트 서버 **m710q**(tailscale serve → `127.0.0.1:8011`) @ **0.1.55**. 이미지 `honestjung/cdgts:0.1.35~0.1.56` dockerhub push 완료. 0.1.53~0.1.55 운영 반영 완료 + 재시드.
...
- **0.1.57 빌드·푸시 완료(미배포) — prod 실배포에서 나온 2 갭 수정**([커밋 9f6f620]): ⓐ **smoke SSL false-fail** — prod `SECURE_SSL_REDIRECT=True` 라 평문 HTTP `/healthz` 가 301 → curl(-L 없음) 빈 본문 → status!=ok 오판. `smoke.sh`·deploy 대기 루프에 `X-Forwarded-Proto: https`(SECURE_PROXY_SSL_HEADER) 헤더. 재현+수정 검증 완료. ⓑ **deploy 에 seed 단계 부재** — 시드 변경/빈 DB 최초 배포 시 smoke 가 재시드 전 실패. `deploy-{prod,dev}.sh X.Y.Z --reseed` → migrate 후 smoke 전 `seed --mode=replace`+`seed_demo`. (0.1.56 prod smoke 실패는 순전히 ⓐ, prod 는 정상.) **다음: prod/test 를 0.1.57 로 올려 fixed 파이프라인 검증**(0.1.56→0.1.57 seed 무변경 → --reseed 불필요). ⚠️ `--reseed` 를 래퍼로 쓰려면 `_extract_and_deploy.sh` 갱신 위해 재부트스트랩(`sync_to_srv.sh`) 1회 필요.
