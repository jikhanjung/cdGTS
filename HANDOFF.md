...
**Last updated**: 2026-07-15. 운영 `cdgts.paleobytes.info`(dolfinid-2) @ **0.1.65** · 테스트 서버 **m710q**(tailscale serve → `127.0.0.1:8011`) @ **0.1.65**. 이미지 `honestjung/cdgts:0.1.35~0.1.65` dockerhub push 완료. 공개 `https://cdgts.paleobytes.info/healthz` = 0.1.65/node_types 17. **양 서버 cdgts + cdgts-worker 둘 다 가동, 비-root(prod uid 1001·test uid 1000), DB = `/srv/cdGTS/db/` 서브디렉터리 마운트**.
...
- **3-repo 배포 일관성 정렬 + hourly DB 백업·배포(0.1.63)**([devlog 147](devlog/20260714_147_three-repo-consistency-align.md)): 계약 정렬 3개 프로젝트(cdGTS·fcmanager·fsis) 배포 스크립트 동형화. `scripts/backup_db.py` 신설(sqlite online backup·12개 유지) → 종전 pre_deploy 스냅샷+daily pull 에 **hourly 트랙 추가**(prod cron 등록 완료). `deploy.sh` DEPLOY_SNAPSHOT 기본 1·기동 대기 `/healthz`·DB 게이트 `manage.py shell -c`. `smoke.sh` python3 JSON+버전 필수. 양 서버 배포·검증. ⚠️ backup_db.py 는 self-heal 지연으로 이번 1회 수동 부트스트랩(다음 배포부터 자동).
...
- **배포/운영** (배포·데이터 계약 P08 — [DEPLOY.md](DEPLOY.md)·[deploy/README.md](deploy/README.md)·[deploy/deploy.toml](deploy/deploy.toml)):
  ...
  - **운영서버** `cdgts.paleobytes.info`(GCP dolfinid-2) @ **0.1.65**(nginx + certbot). 테스트 `127.0.0.1:8011`(m710q) @ **0.1.65**.
