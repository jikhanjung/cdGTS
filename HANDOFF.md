...
**Last updated**: 2026-07-20. 운영 `cdgts.paleobytes.info`(dolfinid-2) @ **0.1.71** · 테스트 서버 **m710q**(tailscale serve → `127.0.0.1:8011`) @ **0.1.71**. 이미지 `honestjung/cdgts:0.1.35~0.1.71` dockerhub push 완료. 공개 `https://cdgts.paleobytes.info/healthz` = 0.1.71/node_types 16(boundaries 177·prod graphs 8[사용자 fork 1]·test graphs 7). **양 서버 cdgts + cdgts-worker 둘 다 가동, 비-root(prod uid 1001·test uid 1000), DB = `/srv/cdGTS/db/` 서브디렉터리 마운트**.
