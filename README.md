...
> 상태: 개념(브레인스토밍) → **구현·배포**. 스키마 v0 를 Django 앱 5개 + React 노드 에디터로 구현, 운영서버 [cdgts.paleobytes.info](https://cdgts.paleobytes.info) 에 **v0.1.2** 배포 중. 개념 코퍼스는 `docs/` 에 그대로 유지됩니다.
...
- **스택**: Django 5.2 + SQLite + DRF + React Flow(Vite). 5개 앱(chrono·nodes·graph·engine·releases) + 프론트 노드 에디터·릴리스 diff 뷰. 백엔드 `pytest` 59 passed.
- **엔진**: 값+provenance 전파(pass-through) · 정합성 게이트 · 값/토폴로지 diff 골격. 계산 커널(numpy/scipy)로 age-depth model 실계산.
- **배포**: Docker 이미지 `honestjung/cdgts`. 운영 [cdgts.paleobytes.info](https://cdgts.paleobytes.info) @ **v0.1.2**, 개발/테스트 @ v0.1.1. 원자적 DB 스냅샷 + NAS 오프사이트 백업(04:00 cron).
