...
- [ ] **상호작용 e2e 스모크 확장** — 현재 Tier2 스모크는 boot/render/login 만 덮음. **컨텍스트 메뉴 열기·그룹
      드릴인(더블클릭)·bake/propose** 등 상호작용을 얇게 추가(드릴인 스모크는 `buildView` 커버 = 일석이조).
      **Editor 훅 추출의 안전망 전제.** ([R02 검토 노트](devlog/20260711_R02_source-code-review.md) 우선순위 재정렬)
- [ ] **Editor.jsx 훅 추출 (분해 2차)** — 1차([devlog 137](devlog/20260711_137_editor-decompose.md))로 순수 뷰
      레이어(`graphView.js`)+컨텍스트 메뉴(`EditorMenu.jsx`) 추출, 990줄. **남은 seam = selection 훅 · graph
      actions(save/evaluate/bake/propose) 훅 · read-only gating** — 상태·클로저 의존 깊어 고위험 → **위 상호작용
      스모크 확보 후** 진행. (지금은 정차 지점으로 충분)
...
- [ ] **devlog 시리즈** 단조증가 — `NNN`(완료, 현재 110)·`PNN`(Plan, 현재 P05)·`RNN`(Review, 현재 R01).
