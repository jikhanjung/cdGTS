...
- [x] **매시 백업 무결성 게이트**([devlog 150](devlog/20260715_150_hourly-integrity-gate.md), 0.1.68 양 서버 배포·실측 검증) —
      스냅샷마다 `integrity_check` → 실패 시 **채택 안 함 + prune 중단**(로테이션 오염 방지) + `db/INTEGRITY_FAIL`
      센티넬 → healthz `degraded`(200) → smoke 실패. 열린 후속:
  - [x] **~~3-repo 포팅~~ (무결성 게이트)** — **fcmanager 0.6.24 · fsis2026 0.5.85 가 같은 날 뒤따라 3/5 적용**.
        cdGTS 파일럿→이식이 아니라 **각자 이식**했고 셋이 서로를 몰랐다(계약 문서가 채널 역할을 했다는 증거).
        남은 건 paleonews·naverland(둘 다 prod=m710q 자기호스팅).
  - [ ] **반출 위생 포팅**([devlog 151](devlog/20260715_151_egress-sanitize-session-tokens.md)) — 운영 DB 를 호스트 밖으로
        반출하는 **3/5** 중 cdGTS·fcmanager(0.6.25) 적용, **fsis2026 작업 중**(2026-07-15 기준). cdGTS 쪽 할 일은 없고,
        계약 §롤아웃의 이식 체크리스트가 권위 소스 — 특히 🔴 **`VACUUM` 은 상시 경로에도**(151 §9 에서 내가 빠뜨렸던 자리).
  - [ ] **hourly 보존 24 포팅** — cdGTS 만 24(0.1.69), 나머지 4개는 12 라 **같은 granularity 갭**을 갖고 있다.
        관계식(`RETAIN × 주기 ≥ 오프사이트 간격`)은 계약 MUST 로 올렸고 5개 전부 hourly+daily 라 값은 24 로 공통.
  - [ ] **`rollback.sh --db=restore` 가 복원 대상을 검사하지 않음** — 오염된 스냅샷을 고르면 손상을 되살린다.
        이번 게이트가 오염 자체를 막아 위험은 줄었으나, 복원 직전 `integrity_check` 1회는 값싼 방어.
  - [ ] **`MAILTO` 미설정**(prod crontab) — 센티넬은 **배포 시점**에만 사람에게 닿는다. 배포가 뜸하면 탐지도 뜸함
        (prune 보존은 배포와 무관하게 동작하므로 안전망 자체는 유효). 3-repo 공용 호스트 설정이라 별건.
  - [ ] **NAS 백업 디렉터리가 `drwxrwxrwx`(0777)** ([devlog 151](devlog/20260715_151_egress-sanitize-session-tokens.md) §9) —
        세션 토큰은 반출 위생으로 뺐지만(151), 강한 해시·그래프 데이터는 여전히 world-readable 에 90일 산다.
        *무엇을 내보내나*는 고쳤고 *어디에 두나*는 그대로. 별건 검토(NAS 공유 설정은 3-repo 공용).
  - [ ] **`preflight.sh` 의 "host 스크립트 변경 → sync_to_srv.sh 필요"는 낡은 휴리스틱** — self-heal(0.1.58) 이전
        산물이라 릴리스마다 거짓 경고. 실제로는 이미지 추출로 자동 갱신됨(0.1.68 에서 `backup_db.py` self-heal 실측 확인).
...
- [ ] **devlog 시리즈** 단조증가 — `NNN`(완료, 현재 149)·`PNN`(Plan, 현재 P08)·`RNN`(Review, 현재 R05).
