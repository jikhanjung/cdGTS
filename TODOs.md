...
- [~] **`engine._certify` 층서순 정합** — **authored `order` 제약**(선후 검사 노드)이 있으면 L1 을 그걸로
      판정(devlog 040). authored 제약 없을 때의 게이트웨이 나열순 휴리스틱은 여전히 스텁 → 연대순 정렬 후속.
...
- [ ] **devlog 시리즈** 단조증가 — `NNN`(완료, 현재 038)·`PNN`(Plan, P02)·`RNN`(Review, 미도입).
