...
- [~] **아크 C — 멀티유저 "CI for science" 플랫폼**: **MVP 구현 완료(미배포)**.
      **[P04](devlog/20260707_P04_editor-bake-vault-restructure.md) ✅** Editor→Bake→**Vault**(불변 Release 1급,
      nav Editor·Vault, devlog 102~104, 테스트 0.1.26). **[P05](devlog/20260707_P05_arc-c-multiuser-ci-platform.md)**
      — .1 인증(세션·Membership)·.2 소유/가시성·.3 fork·.4 propose/review/ratify(devlog 106~109) **✅**.
      로그인→fork→편집→(bake→Vault)/propose→review→ratify 작동. 남음: P05.5 샌드박스 오버라이드(아크 B seam),
      인터벌 스코프 권한(훅 준비됨), 배포(관리자·ICS Authority·Membership 세팅).
...
- [ ] **devlog 시리즈** 단조증가 — `NNN`(완료, 현재 109)·`PNN`(Plan, 현재 P05)·`RNN`(Review, 현재 R01).
