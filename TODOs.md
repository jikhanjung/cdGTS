...
- [ ] **브라우저 육안 검증** — 프론트 drag&drop·엣지·복원·결과뱃지·diff 뷰(헤드리스 미검증분).
...
- [x] **운영/개발 분리 배포** — 운영 `cdgts.paleobytes.info` @ 0.1.2(nginx+certbot), 개발/테스트 @ 0.1.1.
...
- [ ] **운영 재시드 실행** — 위 도구를 담은 이미지 배포 후 운영서버에서 `seed --mode=replace` 1회(운영 DB 드리프트 해소).
- [ ] **운영 배포 검증 기록** — 재시드 후 헬스체크·seed 확인 결과를 devlog(done-log)에 정리.
