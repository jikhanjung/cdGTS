...
- [ ] **아크 C — 멀티유저 "CI for science" 플랫폼**: 인증·소유·샌드박스 권한 · fork/PR 워크플로우.
      ...
      **선행: [P04](devlog/20260707_P04_editor-bake-vault-restructure.md)** — Editor→Bake→**Vault** 재구성
      (아티팩트=불변 Release를 1급으로; nav를 Editor·Vault 2개로). **계획: [P05](devlog/20260707_P05_arc-c-multiuser-ci-platform.md)**
      — 인증→소유/가시성→fork→propose/review(verify diff 재사용)→ratify. MVP = 로그인·fork·bake·diff 리뷰.
...
- [ ] **devlog 시리즈** 단조증가 — `NNN`(완료, 현재 101)·`PNN`(Plan, 현재 P05)·`RNN`(Review, 현재 R01).
