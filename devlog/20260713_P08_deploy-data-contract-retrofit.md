...
### P08.6 — 운영 git pull 제거 — ✅ 완료([devlog 143](20260713_143_git-free-deploy.md))

> **구현됨(2026-07-13).** host 운영 파일은 이미 `COPY . .` 로 이미지 `/app/deploy/host/*` 에 실려 있었다 →
> `deploy-{prod,dev}.sh` 를 **git-free 부트스트랩**으로 재작성: 공유 `_extract_and_deploy.sh` 가 이미지에서
> compose·deploy.sh·smoke·rollback·maintenance 를 `docker cp` 로 추출 후 deploy.sh 로 위임. 호스트 상시 파일 =
> `.env`+래퍼 2개+`_extract_and_deploy.sh`. `sync_to_srv.sh` 는 부트스트랩 전용으로 강등. 아래는 원안.
...
**P08 전체 완료**(devlog 140·141·142·143). 레인 경계·매니페스트·동사·헬스·git-free 배포 모두 착지.
**0.1.56 릴리스로 실배포 검증** — 특히 seed replace 운영 데이터 보존·[6/6] smoke·git-free 추출은 실배포에서 최종 확인.
