...
- **auto-evaluate 렌더** — 기본 그래프(Example ① Precambrian GSSA) 로드 시 base-proterozoic 이 **2500 Ma** 로
  표시되는지. `runEvaluation`(평가+결과부착) 전 경로를 브라우저에서 실동작 검증(Tier1 이 백엔드에서 덮는 것의 프론트 짝).
- **그룹 드릴인** — Example ④(그룹 15개) 선택 → 그룹 노드 더블클릭 → "Inside group" 브레드크럼 + Exit 컨트롤.
  **1차에서 추출한 `buildView` + activeGroup 상태를 브라우저에서 커버**(일석이조).
