...
- 순서가 값을 **실제로 조정**해야 한다면, 그건 그래프 순회 방향이 아니라 **coherence gate의 재조정(L3b) /
  ...
  > ⚠️ **재검토(2026-07):** clamp는 별도 개념에서 축소됨 — 사이클 절단은 `Clamp` 프리미티브가 아니라 authored `published-age` **leaf** / joint-inference 노드 / `order` 엣지로 접힌다. 근거: [cycles.md §12](cycles.md#12-재검토-노트-2026-07--clamp는-별도-개념으로-필요한가).
