...
- **order(단조성) → 이미 order 엣지(L1 게이트)가 담당.** `Clamp{order}`는 중복.
  - **후일담(devlog 149)**: 이 결론이 `releases.Clamp{order}` 만 걷어냈을 뿐, 그래프 쪽 **`order` NodeType**
    (+커널 +L1 폴백 가지)은 남아 있었다 — 어떤 그래프도 쓰지 않는 채로. 이제 그것도 제거했고, `order` 가
    **clamp 카테고리의 마지막 멤버**였으므로 **`clamp` 카테고리 자체가 사라졌다**(`NodeType.Category` 에서 제거).
    남은 카테고리 = data · process · reference. → [tier-category-model](tier-category-model.md) §2·§3.
