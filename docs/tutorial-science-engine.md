...
배포된 데모 그래프 두 개는 **값도 오차도 완전히 동일**하고, 딱 하나 — 두 U-Pb 연대가 **하나의 공유 보정
노드**(`decay-238U`)에 걸리느냐, **각자 다른 노드**에 걸리느냐 — 만 다릅니다. 경계마다 사슬은:
...
### 단계
1. **Editor** 화면. 상단 **그래프 선택 드롭다운** → **"Demo: duration overlap (independent decay-const nodes → L1b warn)"**.
2. **Actions ▾ → Evaluate**.
3. 하단 **Results 패널** 확인:
   - consistency 배지 **노랑(warn)**, **L1b** 칩 노랑.
   - 경고줄: `L1b 순서 통계적 미해결: olenekian↔anisian (Δ2.0 < 2σ 4.243)`.
4. 드롭다운 → **"Demo: duration resolved (shared decay-const node → L1b pass)"** → **Evaluate**.
   - consistency **초록(pass)**, **L1b** 초록.
   - 두 그래프를 나란히 보면 차이가 **눈에** 보입니다: warn 쪽은 λ238U 노드가 **둘**, pass 쪽은 **하나**가 두 연대로 갈라집니다.
...
경계는 **base-Olenekian 249.0 Ma**, **base-Anisian 247.0 Ma** (간격 **2.0 Myr**). 각 연대는 자기 **분석오차
1σ ≈ 0.54** 만 authored 하고, 보정 노드가 **계통 σ 1.4** 를 실어 주어 marginal 1σ = √(0.54²+1.4²) = **1.5**.

- **independent(warn)**: 두 연대가 **다른** 보정 노드(ref 다름) → `Cov = 0` → `2σ_gap = 2·√(1.5² + 1.5²) ≈ 4.24`.
  간격 2.0 < 4.24 → **순서 미해결(warn)**. ("2 Myr 떨어졌다지만 오차가 4 Myr라 어느 게 먼저인지 확신 못 함.")
- **shared(pass)**: 두 연대가 **같은** 보정 노드(ref `decay-238U`, σ 1.4)를 공유 → `Cov = 1.4·1.4 = 1.96` →
...
- 공분산 대비 그래프 2개(위 §2) — 값·± 동일, `decay-238U` 보정 노드를 **공유하느냐(하나)/따로냐(둘)** 만 차이.
