...
> 현재 **v0.1.70** — **preliminary·proof-of-concept 단계**의 프로토타입으로, age–depth 계산·제한된
> 불확실성/공분산 전파·bake·diff·제안/검토/비준 흐름이 구현돼 있고, 공유 보정상수 값 변경에 따른 의존 연대
> rescale 과 joint Bayesian 추정은 로드맵에 있습니다. 아래 현재형은 "구현된 slice" 로 읽으시고, 로드맵 항목은
> 그렇게 표시했습니다.
...
flowchart TB
  subgraph r1[" "]
    direction LR
    O["방사·천문·생층서<br/>관측 (leaf·인용)"] --> A["age–depth model<br/>(섹션별 보간)"] --> C["섹션 간 상관<br/>correlation"]
  end
  subgraph r2[" "]
    direction LR
    B["경계 연대"] --> R["bake → 버전 릴리스"] -->|diff| R2["다른 릴리스 /<br/>공표 baseline"]
  end
  C --> B
...
- **경계는 노드**이고, **구간은 두 경계를 묶는 unit node group(span)** 입니다(GSSP=물리적 지점 → 연대는 파생값 ·
  GSSA=약속된 숫자 → 연대가 정의, 화살표가 반대).
- **provenance 가 그래프 1급 시민입니다.** 어떤 연대가 어느 관측·어느 보정상수·어느 논문에서 왔는지 엣지로 남습니다.
  경계를 클릭하면 그 숫자가 서 있는 근거 사슬 전체가 보입니다.
- **불확실성이 배선을 따라 흐릅니다.** 두 연대가 같은 ²³⁸U 붕괴상수를 쓰면, 엔진은 **명시적으로 태깅된 공유
  의존을 이용해 지원되는 계산 경로에서** 두 연대의 공분산을 전파합니다(순진한 독립 가정이 놓치는 부분).
...
같은 그래프에서 두 산출물이 나옵니다 — **ICC = bake**(얼린 차트 스냅샷; 비준 시 권위 릴리스) · **GTS =
narrate**(구조화 필드에서 결정적 템플릿으로 렌더한 서술, LLM 창작 없음).
...
- **초기 proof-of-concept 단계입니다(v0.1.70).** 스키마와 커널은 돌아가지만, 입력 데이터 포맷·통계 절차
  (예: joint 베이지안 추정)는 진화 중입니다.
