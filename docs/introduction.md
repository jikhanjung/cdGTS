...
산출물** 로 다루는 것입니다. 곧 **엔진**입니다 — 데이터·붕괴상수·상관 가설을 바꾸면 하류 연대가 다시 전파돼
**즉시** 결과를 봅니다. 소프트웨어의 CI/CD 에서 빌린 발상, *"과학을 위한 CI"* 입니다.

## 핵심 두 가지

**① 엔진이다 — 입력을 바꾸면 결과가 바로 나온다.** 차트의 모든 숫자는 상류 노드에서 *계산된* 값이지 타이핑한
값이 아닙니다. 방사연대를 수정하고, age–depth 모델을 재튜닝하고, 상관 엣지를 토글하고, 공유 보정상수를 바꾸면,
경계 연대가 의존 그래프를 따라 다시 계산됩니다. 손으로 재유도할 필요도, 낡은 스프레드시트도 없이 — "이걸
받아들이면?" 이 살아 있는, 재현 가능한 결과가 됩니다.
...
움직였고 배선이 어떻게 달라졌는지 요약을 얻습니다. 재현성과 provenance 가 덧붙임이 아니라 기본
내장 — 릴리스는 자신을 만든 그래프를, 각 연대가 매달린 관측·논문까지 기록합니다.
...
- **캄브리아기 base(Base of Cambrian)** — Oman·Namibia·Siberia 세 섹션의 U–Pb ash bed 가 δ¹³C BACE 이상을
  bracket → age-depth → 대륙 간 상관 → *T. pedum* FAD 로 보정 이전 → 경계 연대(≈538.8 Ma). 연대가 **다른
  대륙에서** 오는 상관 주도 사례.
...
1. **"이 새 연대를 받아들이면 무엇이 바뀌나?" 를 원클릭으로.** 그래프를 편집 → 재bake → 공표 baseline(앱에
   시드된 ICC `ICS-2024/12`) 과 **diff**. 어느 경계가 얼마나 움직였고, 어떤 배선이 달라졌는지 요약됩니다.
   개정 사이의 "만약에" 를 손계산이 아니라 재현 가능한 산출로.
...
- **아직 v0 입니다.** 스키마와 커널은 돌아가지만, 입력 데이터 포맷·통계 절차(예: joint 베이지안 추정)는
  진화 중입니다.
