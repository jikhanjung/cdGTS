...
cdGTS는 **"continuously deployed Geologic Time Scale"** 에 대한 프로젝트입니다. **브레인스토밍으로 시작해 실행 가능한 코드베이스로 전환**되었습니다 — 개념 코퍼스(`docs/`)와 구현(Django 앱 + React 프론트)이 함께 있습니다.

- **현재 상태**: 스키마 v0 를 Django 5.2 + DRF + React Flow 로 구현, 운영서버(cdgts.paleobytes.info)에 **v0.1.2** 배포 중. 상세는 **[HANDOFF.md](HANDOFF.md)** (현재 상태) · **[TODOs.md](TODOs.md)** (백로그) · `devlog/` (라운드별 기록).
