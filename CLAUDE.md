...
- **현재 상태**: 스키마 v0 를 Django 5.2 + DRF + React Flow 로 구현, 운영서버(cdgts.paleobytes.info)에 **v0.1.34** 배포 완료. **P04**(불변 Bake·Vault) · **P05**(멀티유저 CI: 인증·소유·fork·propose/ratify) · **P06 Science Engine**(공분산 백본·정합성 게이트·clamp reconcile·캡스톤 데모) 모두 배포됨. 다음은 **P06.4**(베이지안 joint + 워커) 계획 완료·미착수. 상세는 **[HANDOFF.md](HANDOFF.md)** (현재 상태) · **[TODOs.md](TODOs.md)** (백로그) · `devlog/` (라운드별 기록).
